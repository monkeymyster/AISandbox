...
class SearchAndDestroy(commander.Commander):
    ...
    def nearest_unexplored_root_node(self, bot):
        closest_node = None
        closest_distance = 1000000
        for node in self.terminal_nodes:
            if self.explored[node] == False:
                bot_position = self.lookup[int(bot.position.x)][int(bot.position.y)]
                distance = self.corridor_graph.node[bot_position]["distances"][node]
    ...
    def tick(self):
        ...
        for bot in self.game.bots_available:
            node = self.nearest_unexplored_root_node(bot)
